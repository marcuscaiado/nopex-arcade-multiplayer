...
if (masterStatus) {
  execSync('git commit -m "feat(mobile): dynamic floating virtual joystick, on-screen dpad, and tap-to-walk 3D navigation"', { stdio: 'inherit' });
...
if (status) {
  execSync('git commit -m "deploy: 🚀 Production bundle fix & Mobile walkable controls"', { stdio: 'inherit' });
